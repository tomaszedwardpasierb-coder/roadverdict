// Mirrors ExportShareSection.test.tsx for the car equivalent component.
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { act, render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { CarExportShareSection } from "@/app/dashboard/CarExportShareSection";

describe("CarExportShareSection", () => {
  beforeEach(() => {
  ...
  it("pro plan: the real CSV link is shown, pointing at the export route, with no upsell", () => {
    render(<CarExportShareSection isPro={true} />);
  ...
  it("creates a link with the real form state (duration, email, asking price) and renders the result", async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true,
      json: async () => ({ url: "https://roadverdict.app/car-report/tok123", expiresAt: "2024-06-01T00:00:00.000Z" }),
    });
    const user = userEvent.setup();
    render(<CarExportShareSection isPro={false} />);
    await user.type(screen.getByLabelText("Sharing with (email address)"), "buyer@example.com");
    ...
    expect(await screen.findByDisplayValue("https://roadverdict.app/car-report/tok123")).toBeInTheDocument();
  ...
  it("Copy writes the real share URL to the clipboard and reverts its label after a couple of seconds", async () => {
    ...
    expect(await navigator.clipboard.readText()).toBe("https://roadverdict.app/car-report/tok123");
  ...
  it("sends the link by email to the correct per-token endpoint, and clears the field on success", async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(
      "/api/cars/car-share-link/tok123/send-email",
