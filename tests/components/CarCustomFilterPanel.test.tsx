// Place at: tests/components/CarCustomFilterPanel.test.tsx
//
// Car equivalent of CustomFilterPanel.test.tsx - same standalone
// client-side lookup, exercised against the real car catalogs
// (CAR_JOB_LABELS/CAR_MOD_LABELS/CAR_BILL_LABELS).
import { describe, expect, it } from "vitest";
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { CarCustomFilterPanel } from "@/app/dashboard/CarCustomFilterPanel";

const records = [
  { jobType: "oil-filter", date: "2026-01-01", cost: 50 },
  { jobType: "full-service", date: "2026-02-01", cost: 80 },
];
const mods = [
  { category: "dash-cam", name: "Nextbase 622GW", date: "2026-01-10", cost: 40 },
  { category: "security-tracker", name: "Tracker Monitor", date: "2026-02-10", cost: 25 },
];
const bills = [
...
describe("CarCustomFilterPanel", () => {
  ...
  it("selecting a suggested mod from the real search autocomplete narrows to that exact item and its group", async () => {
    ...
    render(<CarCustomFilterPanel {...baseProps()} />);
    await user.selectOptions(screen.getByLabelText("Category"), "mods");
    await user.type(screen.getByLabelText("Search for an item"), "dash cam");
    await user.click(await screen.findByRole("button", { name: "Dash cam" }));
  ...
  it("Last N days computes a real cutoff from the current clock, not a fixed date", async () => {
    ...
    const user = userEvent.setup();
    render(
      <CarCustomFilterPanel
        {...baseProps()}
        records={[
          { jobType: "oil-filter", date: recent, cost: 50 },
