// Mirrors StorySoFarTab.test.tsx for the car equivalent component.
import { afterEach, describe, expect, it, vi } from "vitest";
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { CarStorySoFarTab } from "@/app/dashboard/CarStorySoFarTab";
...
const filledSellerPrep = {
  ...
  likelyQuestions: ["Has it ever been in an accident?"],
...
describe("CarStorySoFarTab", () => {
  ...
  it("with no story yet but real logged history, shows the seller-prep stats, issues, upcoming costs, questions and plan", () => {
    mockFetchRouter(() => Promise.reject(new Error("should not be called")));
    render(
      <CarStorySoFarTab currentMileage={12000} distanceUnit="mi" initialStory={null} sellerPrep={filledSellerPrep} />
    );
    ...
    expect(screen.getByText("Has it ever been in an accident?")).toBeInTheDocument();
    expect(screen.getByText(/Fill the gap in your service history/)).toBeInTheDocument();
  });

  it("shows the car tag from nickname and registration together, separated by a dot", () => {
    mockFetchRouter(() => Promise.reject(new Error("should not be called")));
    render(
      <CarStorySoFarTab
        carNickname="Betty"
  ...
  it("renders the current mileage converted for the display unit", () => {
    mockFetchRouter(() => Promise.reject(new Error("should not be called")));
    render(
      <CarStorySoFarTab currentMileage={10000} distanceUnit="km" initialStory={null} sellerPrep={emptySellerPrep} />
    );
  ...
  it("clicking Generate fetches a fresh story and renders it, including the AI-hidden owner notes", async () => {
    mockFetchRouter(() =>
      Promise.resolve({
        ...
        json: async () => ({
          ...
          sharedStory: ["This car has a clean, well-documented history."],
    ...
    expect(await screen.findByText("This car has a clean, well-documented history.")).toBeInTheDocument();
  ...
  it("an already-cached, persisted story renders directly from initialStory with no fetch on mount", () => {
    ...
    const initialStory = {
      generatedWithAi: true,
      sharedStory: ["A steady runabout with regular servicing."],
      ownerNotes: [],
    ...
    render(
      <CarStorySoFarTab currentMileage={12000} distanceUnit="mi" initialStory={initialStory} sellerPrep={emptySellerPrep} />
    );

    expect(screen.getByText("A steady runabout with regular servicing.")).toBeInTheDocument();
    expect(screen.queryByText("For you only - never shown to a buyer")).not.toBeInTheDocument();
    expect(fetch).not.toHaveBeenCalledWith("/api/cars/car-story-so-far");
  });
  ...
  it("posts to the car-specific story endpoint, not the bike one", async () => {
    const fetchMock = vi.fn((url: string) => {
      if (url === "/api/tracker/notifications") return notificationsOkResponse();
      return Promise.resolve({
        ok: true,
        json: async () => ({
          generatedWithAi: false, sharedStory: ["x"], ownerNotes: [],
          verdict: { tier: "strong", label: "Well documented" },
          generatedAt: "2026-08-25T00:00:00.000Z", cached: false, nextAvailableAt: "2026-09-01T00:00:00.000Z",
        }),
      });
    });
