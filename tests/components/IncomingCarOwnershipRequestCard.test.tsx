// Mirrors IncomingOwnershipRequestCard.test.tsx for the car equivalent component.
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { IncomingCarOwnershipRequestCard } from "@/app/dashboard/IncomingCarOwnershipRequestCard";

describe("IncomingCarOwnershipRequestCard", () => {
  beforeEach(() => {
  ...
  it("renders the requester's email and formatted request date", () => {
    render(<IncomingCarOwnershipRequestCard requestId="req-1" requesterEmail="buyer@example.com" createdAt="2026-03-05" />);
  ...
  it("approving with records included posts includeRecords:true and shows the records-transferred message", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/cars/car-transfer/incoming/req-1/approve",
  ...
  it("declining sends no body and shows the declined message", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith("/api/cars/car-transfer/incoming/req-1/decline", expect.objectContaining({ method: "POST" }));
  ...
  it("shows the car spinner on Approve while the request is in flight", async () => {
