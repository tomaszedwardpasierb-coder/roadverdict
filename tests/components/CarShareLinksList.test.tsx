// Mirrors ShareLinksList.test.tsx for the car equivalent component.
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { render, screen } from "@testing-library/react";
import userEvent from "@testing-library/user-event";
import { CarShareLinksList } from "@/app/dashboard/CarShareLinksList";
...
const carNames = { car1: "Ford Focus", car2: "Vauxhall Corsa" };
const appUrl = "https://roadverdict.example";

describe("CarShareLinksList", () => {
  beforeEach(() => {
  ...
  it("shows the empty-state note when there are no links yet", () => {
    render(<CarShareLinksList links={[]} carNames={carNames} appUrl={appUrl} />);
    expect(screen.getByText(/No shareable links generated yet/)).toBeInTheDocument();
  });

  it("renders each real link's car name, recipient, asking price, url and expiry, tagging only the expired one", () => {
    render(<CarShareLinksList links={links} carNames={carNames} appUrl={appUrl} />);
    ...
    expect(screen.getByText(`${appUrl}/car-report/tok1`)).toBeInTheDocument();
  ...
  it("falls back to 'Unknown car' for a link whose car no longer resolves", () => {
    render(<CarShareLinksList links={[links[0]]} carNames={{}} appUrl={appUrl} />);
    expect(screen.getByText("Unknown car")).toBeInTheDocument();
  });

  it("Copy link writes the real report URL to the clipboard", async () => {
    const user = userEvent.setup();
    const writeText = vi.spyOn(navigator.clipboard, "writeText").mockResolvedValue(undefined);
    render(<CarShareLinksList links={[links[0]]} carNames={carNames} appUrl={appUrl} />);

    await user.click(screen.getByRole("button", { name: "Copy link" }));
    expect(writeText).toHaveBeenCalledWith(`${appUrl}/car-report/tok1`);
  });
  ...
  it("Delete, once confirmed, DELETEs the link's own endpoint and refreshes the page", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith("/api/cars/car-share-link/tok1", expect.objectContaining({ method: "DELETE" }));
  ...
  it("Extend posts the chosen duration and refreshes on success", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/cars/car-share-link/tok1/extend",
  ...
  it("Add price on a link with none yet starts from an empty input and posts the new price", async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValue({ ok: true });
    const user = userEvent.setup();
    render(<CarShareLinksList links={[links[1]]} carNames={carNames} appUrl={appUrl} />);
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/cars/car-share-link/tok2/asking-price",
  ...
  it("Edit price on a link that already has one is pre-filled, and clearing it sends null to clear it server-side", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "/api/cars/car-share-link/tok1/asking-price",
