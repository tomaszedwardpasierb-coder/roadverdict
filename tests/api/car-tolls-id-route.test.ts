...
vi.mock("@/lib/tracker/carToll", () => ({ updateCarToll: mocks.updateCarToll, deleteCarToll: mocks.deleteCarToll }));

import { PATCH, DELETE } from "@/app/api/cars/car-tolls/[id]/route";

function request(body?: string): NextRequest {
  return new NextRequest("http://localhost/api/cars/car-tolls/x", {
...
describe("PATCH /api/cars/car-tolls/[id]", () => {
  ...
  it("refuses an id prefixed with a different owner's email, without ever calling updateCarToll", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await PATCH(request(JSON.stringify(validPayload)), { params: Promise.resolve({ id: "attacker@example.com::carToll::abc123" }) });
    expect(response.status).toBe(404);
    expect(mocks.updateCarToll).not.toHaveBeenCalled();
  });
  ...
  it("returns not found when the update itself finds nothing to update", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    mocks.updateCarToll.mockResolvedValue(null);
    const response = await PATCH(request(JSON.stringify(validPayload)), { params: Promise.resolve({ id: ownId }) });
    expect(response.status).toBe(404);
  });
...
describe("DELETE /api/cars/car-tolls/[id]", () => {
  beforeEach(() => {
    Object.values(mocks).forEach((mock) => mock.mockReset());
    mocks.getPrimaryCar.mockResolvedValue({ id: "car-1" });
    mocks.isCarReadOnly.mockReturnValue(false);
  ...
  it("refuses an id prefixed with a different owner's email, without ever calling deleteCarToll", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await DELETE(request(), { params: Promise.resolve({ id: "attacker@example.com::carToll::x" }) });
    expect(response.status).toBe(404);
    expect(mocks.deleteCarToll).not.toHaveBeenCalled();
  });
  ...
  it("deletes a valid, owned toll", async () => {
    ...
    const response = await DELETE(request(), { params: Promise.resolve({ id: ownId }) });
    expect(response.status).toBe(200);
    expect(mocks.deleteCarToll).toHaveBeenCalledWith("owner@example.com", ownId);
  });
});
