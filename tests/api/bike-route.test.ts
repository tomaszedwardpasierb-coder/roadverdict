...
const mocks = vi.hoisted(() => ({
  ...
  updateBikeIncludeCleaningInReport: vi.fn(),
...
vi.mock("@/lib/tracker/bike", async () => {
  ...
  return {
    ...
    updateBikeIncludeCleaningInReport: mocks.updateBikeIncludeCleaningInReport,
...
describe("POST /api/tracker/bike", () => {
  ...
  // See bill.ts's logVedBillIfNeeded - a confirmed-taxed vehicle also
  // gets its current VED period logged as a real expense at creation
  // time, not just on a later "Refresh vehicle data" click.
  ...
  // See reminder.ts's syncSornReminder - a taxed vehicle with a due date
  // gets a "Road tax renewal due" reminder synced too, not just the SORN
  // reminder.
  it("passes the tax due date through to syncSornReminder for a taxed bike", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
  ...
  it("skips the tax/SORN check entirely when VDG_API_KEY isn't configured", async () => {
    ...
    expect(mocks.logVedBillIfNeeded).not.toHaveBeenCalled();
  });
...
describe("PATCH /api/tracker/bike", () => {
  ...
  it("updates includeCleaningInReport independently of the other report toggles", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    mocks.updateBikeIncludeCleaningInReport.mockResolvedValue({ id: "bike-1", includeCleaningInReport: true });
    const response = await PATCH(request("PATCH", JSON.stringify({ includeCleaningInReport: true })));
    expect(response.status).toBe(200);
    expect(mocks.updateBikeIncludeCleaningInReport).toHaveBeenCalledWith("owner@example.com", "bike-1", true);
    expect(mocks.updateBikeIncludeTollsInReport).not.toHaveBeenCalled();
  });
