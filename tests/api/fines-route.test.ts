...
vi.mock("@/lib/tracker/fine", () => ({ createFine: mocks.createFine }));

import { POST } from "@/app/api/tracker/fines/route";

function request(body: string): NextRequest {
  return new NextRequest("http://localhost/api/tracker/fines", {
    method: "POST",
...
const validPayload = { fineType: "speeding", cost: 100, date: "2025-06-01", notes: "M1 southbound" };
...
describe("POST /api/tracker/fines", () => {
  ...
  it("rejects a fine dated before the bike's production year", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
  ...
  it("creates a valid fine", async () => {
    ...
    expect(mocks.createFine).toHaveBeenCalledWith("owner@example.com", expect.objectContaining({ bikeId: "bike-1", fineType: "speeding", cost: 100 }));
  });
});
