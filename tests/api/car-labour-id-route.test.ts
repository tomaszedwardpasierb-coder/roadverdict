...
const mocks = vi.hoisted(() => ({
  ...
  isCarReadOnly: vi.fn(),
  updateCarLabour: vi.fn(),
  deleteCarLabour: vi.fn(),
  getCarLabour: vi.fn(),
...
vi.mock("@/lib/tracker/carLabour", () => ({ updateCarLabour: mocks.updateCarLabour, deleteCarLabour: mocks.deleteCarLabour, getCarLabour: mocks.getCarLabour }));
...
import { PATCH, DELETE } from "@/app/api/cars/car-labour/[id]/route";

function request(body?: string): NextRequest {
  return new NextRequest("http://localhost/api/cars/car-labour/x", {
...
const ownId = "owner@example.com::carLabour::abc123";
const validPayload = { category: "hv-battery-health-check", cost: 60, mileage: 42000, date: "2025-06-01" };

describe("PATCH /api/cars/car-labour/[id]", () => {
  beforeEach(() => {
    ...
    mocks.updateCarLabour.mockResolvedValue({ id: ownId });
  });
  ...
  it("refuses an id prefixed with a different owner's email, without ever calling updateCarLabour", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await PATCH(request(JSON.stringify(validPayload)), { params: Promise.resolve({ id: "attacker@example.com::carLabour::x" }) });
    expect(response.status).toBe(404);
    expect(mocks.updateCarLabour).not.toHaveBeenCalled();
  });
  ...
  it.each(["estimated", "interpolated"])("promotes a %s mileage confidence to confirmed on edit", async (confidence) => {
    ...
    expect(mocks.updateCarLabour).toHaveBeenCalledWith("owner@example.com", ownId, expect.objectContaining({ mileageConfidence: "confirmed" }));
  });
  ...
  it("returns not found when the update itself finds nothing to update", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    mocks.updateCarLabour.mockResolvedValue(null);
    const response = await PATCH(request(JSON.stringify(validPayload)), { params: Promise.resolve({ id: ownId }) });
    expect(response.status).toBe(404);
  });
});

describe("DELETE /api/cars/car-labour/[id]", () => {
  beforeEach(() => {
    Object.values(mocks).forEach((mock) => mock.mockReset());
  ...
  it("refuses an id prefixed with a different owner's email", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await DELETE(request(), { params: Promise.resolve({ id: "attacker@example.com::carLabour::x" }) });
    expect(response.status).toBe(404);
    expect(mocks.deleteCarLabour).not.toHaveBeenCalled();
  });
  ...
  it("deletes a valid, owned car labour entry", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await DELETE(request(), { params: Promise.resolve({ id: ownId }) });
    expect(response.status).toBe(200);
    expect(mocks.deleteCarLabour).toHaveBeenCalledWith("owner@example.com", ownId);
  });
});
