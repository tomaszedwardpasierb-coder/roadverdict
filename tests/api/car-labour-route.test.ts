...
vi.mock("@/lib/tracker/carLabour", () => ({ createCarLabour: mocks.createCarLabour, getCarLabour: mocks.getCarLabour }));
...
import { POST } from "@/app/api/cars/car-labour/route";

function request(body: string): NextRequest {
  return new NextRequest("http://localhost/api/cars/car-labour", {
    method: "POST",
...
describe("POST /api/cars/car-labour", () => {
  beforeEach(() => {
  ...
  it("rejects incomplete payloads before accessing the vehicle repository", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await POST(request(JSON.stringify({ category: "hv-battery-health-check" })));
    expect(response.status).toBe(400);
  ...
  it("blocks writes to a transferred vehicle", async () => {
    ...
    const response = await POST(request(JSON.stringify({
      category: "hv-battery-health-check", cost: 60, mileage: 40000, date: "2025-01-01",
    })));
    ...
    expect(mocks.createCarLabour).not.toHaveBeenCalled();
  });

  it("creates a valid labour entry for the authenticated owner's active car", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    mocks.getPrimaryCar.mockResolvedValue({ id: "car-1", currentMileage: 40000 });
    mocks.isCarReadOnly.mockReturnValue(false);
    mocks.createCarLabour.mockResolvedValue({ id: "labour-1" });

    const response = await POST(request(JSON.stringify({
      category: "hv-battery-health-check", cost: 60, mileage: 42000, date: "2025-01-01", notes: "Annual check",
    })));

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ labour: { id: "labour-1" } });
    expect(mocks.createCarLabour).toHaveBeenCalledWith("owner@example.com", expect.objectContaining({
      carId: "car-1", category: "hv-battery-health-check", mileage: 42000,
    }));
    expect(mocks.updateCarMileage).toHaveBeenCalledWith("owner@example.com", "car-1", 42000);
  });

  it("folds getCarLabour into the mileage-consistency history alongside records/fuelLogs/mods", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    mocks.getPrimaryCar.mockResolvedValue({ id: "car-1", currentMileage: 40000 });
    mocks.isCarReadOnly.mockReturnValue(false);
    mocks.getCarLabour.mockResolvedValue([{ id: "labour-9", date: "2025-01-01", mileage: 39000 }]);
    mocks.createCarLabour.mockResolvedValue({ id: "labour-1" });

    await POST(request(JSON.stringify({
      category: "hv-battery-health-check", cost: 60, mileage: 42000, date: "2025-01-01",
    })));

    expect(mocks.checkMileageConsistency).toHaveBeenCalledWith(
      42000,
      "2025-01-01",
      expect.arrayContaining([expect.objectContaining({ id: "labour-9", mileage: 39000 })]),
      40000
    );
  });
  ...
  it("returns a conflict when the server-side mileage check rejects the entry", async () => {
    ...
    const response = await POST(request(JSON.stringify({
      category: "hv-battery-health-check", cost: 60, mileage: 39000, date: "2025-01-01",
    })));
