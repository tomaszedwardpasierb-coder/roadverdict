// Mirrors bill-series-route.test.ts for the car equivalent route.
...
vi.mock("@/lib/tracker/carBillSeries", () => ({
  createCarBillSeries: mocks.createCarBillSeries,
  materializeDueInstalments: mocks.materializeDueInstalments,
  materializeExactCount: mocks.materializeExactCount,
}));
vi.mock("@/lib/tracker/billSeriesSchedule", async () => {
  const actual = await vi.importActual<typeof import("@/lib/tracker/billSeriesSchedule")>("@/lib/tracker/billSeriesSchedule");
  return { ...actual, seriesEndDate: mocks.seriesEndDate };
});
...
// productionYearCheck.ts, billTypes.ts, and carBillTypes.ts are deliberately
// NOT mocked - all pure, no I/O, same reasoning as the bike route's own tests.

import { POST } from "@/app/api/cars/car-bill-series/route";

function request(body: string): NextRequest {
  return new NextRequest("http://localhost/api/cars/car-bill-series", {
    method: "POST",
...
describe("POST /api/cars/car-bill-series", () => {
  ...
  it("rejects a start date before the car's production year", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
  ...
  it("creates the series, materialises whatever's due immediately, and creates a renewal reminder", async () => {
    ...
    expect(mocks.createCarBillSeries).toHaveBeenCalledWith("owner@example.com", expect.objectContaining({
      carId: "car-1", billType: "insurance", instalmentAmount: 42.5, depositAmount: 110,
    }));
    expect(mocks.materializeDueInstalments).toHaveBeenCalledWith("owner@example.com", { id: "series-1", carId: "car-1" });
    expect(mocks.deleteCarRemindersBySourceKey).toHaveBeenCalledWith("owner@example.com", "car-1", "bill-series:series-1");
  ...
  it("materialises by explicit count, not date arithmetic, when instalmentsAlreadyPaid is given", async () => {
    ...
    expect(response.status).toBe(200);
    expect(mocks.materializeExactCount).toHaveBeenCalledWith("owner@example.com", { id: "series-1", carId: "car-1" }, 3);
  ...
  it("never sends a deposit through for a road-tax plan, even if the client didn't", async () => {
    ...
    const payload = mocks.createCarBillSeries.mock.calls[0][1];
