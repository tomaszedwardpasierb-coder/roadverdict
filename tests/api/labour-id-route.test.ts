...
vi.mock("@/lib/tracker/labour", () => ({ updateLabour: mocks.updateLabour, deleteLabour: mocks.deleteLabour, getLabour: mocks.getLabour }));
...
import { PATCH, DELETE } from "@/app/api/tracker/labour/[id]/route";

function request(body?: string): NextRequest {
  return new NextRequest("http://localhost/api/tracker/labour/x", {
...
const ownId = "owner@example.com::labour::abc123";
const validPayload = { category: "brake-bleeding", cost: 45, mileage: 5200, date: "2025-06-01" };

describe("PATCH /api/tracker/labour/[id]", () => {
  beforeEach(() => {
    ...
    mocks.updateLabour.mockResolvedValue({ id: ownId });
  });
  ...
  it("refuses an id prefixed with a different owner's email, without ever calling updateLabour", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await PATCH(request(JSON.stringify(validPayload)), { params: Promise.resolve({ id: "attacker@example.com::labour::x" }) });
    expect(response.status).toBe(404);
    expect(mocks.updateLabour).not.toHaveBeenCalled();
  });
  ...
  it.each(["estimated", "interpolated"])("promotes a %s mileage confidence to confirmed on edit", async (confidence) => {
    ...
    expect(mocks.updateLabour).toHaveBeenCalledWith("owner@example.com", ownId, expect.objectContaining({ mileageConfidence: "confirmed" }));
  });
  ...
  it("returns not found when the update itself finds nothing to update", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    mocks.updateLabour.mockResolvedValue(null);
    const response = await PATCH(request(JSON.stringify(validPayload)), { params: Promise.resolve({ id: ownId }) });
    expect(response.status).toBe(404);
  });
});

describe("DELETE /api/tracker/labour/[id]", () => {
  beforeEach(() => {
    Object.values(mocks).forEach((mock) => mock.mockReset());
  ...
  it("refuses an id prefixed with a different owner's email", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await DELETE(request(), { params: Promise.resolve({ id: "attacker@example.com::labour::x" }) });
    expect(response.status).toBe(404);
    expect(mocks.deleteLabour).not.toHaveBeenCalled();
  });
  ...
  it("deletes a valid, owned labour entry", async () => {
    mocks.getSession.mockResolvedValue({ email: "owner@example.com" });
    const response = await DELETE(request(), { params: Promise.resolve({ id: ownId }) });
    expect(response.status).toBe(200);
    expect(mocks.deleteLabour).toHaveBeenCalledWith("owner@example.com", ownId);
  });
});
