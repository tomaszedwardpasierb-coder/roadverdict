// Mirrors billSeries.test.ts for the car equivalent.
...
vi.mock("@/lib/tracker/car", () => ({
  queryCarTrackerDocs: mocks.queryCarTrackerDocs,
}));
...
import {
  ...
  materializeAllDueForCar,
  type CarBillSeriesDoc,
} from "@/lib/tracker/carBillSeries";

const email = "owner@example.com";
const carId = "car-1";

const baseSeries: CarBillSeriesDoc = {
  id: `${email}::carBillSeries::1`,
  pk: email,
  type: "carBillSeries",
  carId,
  billType: "insurance",
...
describe("createCarBillSeries", () => {
  it("delegates to createTrackerDoc with idPrefix and type both 'carBillSeries'", async () => {
    await createCarBillSeries(email, {
      carId, billType: "insurance", frequency: "monthly", startDate: "2025-01-01",
    ...
    expect(mocks.createTrackerDoc).toHaveBeenCalledWith(
      email, "carBillSeries", "carBillSeries",
  ...
  it("starts lastMaterializedIndex at -1 and status 'active'", async () => {
    await createCarBillSeries(email, {
      carId, billType: "road-tax", frequency: "six-monthly", startDate: "2025-01-01",
  ...
  it("sets the base doc's `date` field to startDate", async () => {
    await createCarBillSeries(email, {
      carId, billType: "insurance", frequency: "monthly", startDate: "2025-03-15",
...
describe("getBillSeriesForCar", () => {
  it("queries series scoped to email, type, and carId", async () => {
    await getBillSeriesForCar(email, carId);
    expect(mocks.queryCarTrackerDocs).toHaveBeenCalledWith(email, "carBillSeries", carId);
  });
});

describe("endCarBillSeries", () => {
  it("sets status to 'ended' via updateTrackerDoc", async () => {
    await endCarBillSeries(email, baseSeries.id);
...
describe("materializeDueInstalments", () => {
  ...
  it("writes a bill with a deterministic id keyed on series id + index, not a timestamp", async () => {
    ...
    expect(doc.id).toBe(`${email}::carBill::series::${baseSeries.id}::0`);
  });
...
describe("materializeAllDueForCar", () => {
  ...
  it("does nothing when there are no series for this car", async () => {
    await materializeAllDueForCar(email, carId);
    expect(mocks.upsert).not.toHaveBeenCalled();
  });
});
