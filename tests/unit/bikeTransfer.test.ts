...
beforeEach(() => {
  ...
  // directly. Routing the mocked read() through the existing getBike()
  // mock (with a synthetic _etag attached) and the mocked replace()
  // through the same `upsert` spy new/old-car ordering assertions
  // already rely on means every existing test below keeps working
  // unchanged - only the write mechanism for the old bike moved from a
  // blind items.upsert() to an etag-conditioned item().replace().
  mocks.getContainer.mockReturnValue({
    item: (id: string, pk: string) => ({
      read: async () => {
        const doc = await mocks.getBike(pk, id);
...
describe("transferBike", () => {
  ...
  // bike, and correctly reports the loss - critically, WITHOUT ever
  // creating a duplicate new bike doc (no upsert call at all).
  it("returns already_transferred, without creating a duplicate new bike, when a concurrent transfer wins the race at write time", async () => {
    mocks.getBike
      .mockResolvedValueOnce({ ...oldBike })
      .mockResolvedValueOnce({
        ...oldBike,
        transferredTo: { newBikeId: "winning-racers-new-bike", newOwnerEmail: "other@example.com", transferredAt: "2025-01-01" },
      });
    mocks.getContainer.mockReturnValue({
      item: (id: string, pk: string) => ({
        read: async () => {
