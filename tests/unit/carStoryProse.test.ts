// Mirrors storyProse.test.ts for the car equivalent generator.
...
import { generateCarStoryProse, type CarStoryProseInput } from "@/lib/tracker/carStoryProse";

function geminiResponse(bodyText: string) {
  return { ok: true, json: async () => ({ candidates: [{ content: { parts: [{ text: bodyText }] } }] }) };
}

const validResult = { sharedStory: ["A well-kept car."], ownerNotes: [] };

const baseInput: CarStoryProseInput = {
  identity: {
    year: 2018, make: "Ford", model: "Focus", currentMileage: 15000,
    loggedSinceDate: "2020-01-01", loggedSpanYears: 5, totalLoggedEvents: 20,
  } as any,
  categorySpend: [
    { category: "Servicing", total: 500, count: 3 },
    { category: "Tyres", total: 0, count: 0 },
  ] as any,
...
describe("generateCarStoryProse", () => {
  beforeEach(() => mocks.logGeminiUsage.mockReset());
  afterEach(() => vi.unstubAllGlobals());

  it("logs Gemini usage under the carStoryProse task on success", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(geminiResponse(JSON.stringify(validResult))));
    await generateCarStoryProse(baseInput, "key");
    expect(mocks.logGeminiUsage).toHaveBeenCalledWith("carStoryProse", expect.any(String), true);
  });

  it("logs Gemini usage as a failure on a non-ok HTTP response", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: false, json: async () => ({}) }));
    await generateCarStoryProse(baseInput, "key");
    expect(mocks.logGeminiUsage).toHaveBeenCalledWith("carStoryProse", expect.any(String), false);
  });

  it("returns the parsed story on a well-formed response", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(geminiResponse(JSON.stringify(validResult))));
    expect(await generateCarStoryProse(baseInput, "key")).toEqual(validResult);
  });

  it("fails soft to null on a non-ok HTTP response", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: false, json: async () => ({}) }));
    expect(await generateCarStoryProse(baseInput, "key")).toBeNull();
  });
  ...
  it("fails soft to null when the parsed shape is missing a required array", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(geminiResponse(JSON.stringify({ sharedStory: ["x"] }))));
    expect(await generateCarStoryProse(baseInput, "key")).toBeNull();
  });
  ...
  it("filters non-string entries out of both arrays", async () => {
    ...
    expect(await generateCarStoryProse(baseInput, "key")).toEqual({ sharedStory: ["real"], ownerNotes: ["also real"] });
  });
  ...
  it("includes fuel-efficiency numbers, including the anomaly-exclusion note, when there is enough data", async () => {
    ...
    const fetchMock = vi.fn().mockResolvedValue(geminiResponse(JSON.stringify(validResult)));
    vi.stubGlobal("fetch", fetchMock);
    await generateCarStoryProse(input, "key");
    const prompt = JSON.parse(fetchMock.mock.calls[0][1].body).contents[0].parts[0].text;
  ...
  it("labels unconfirmed-findings facts as owner-only, never shown to a buyer, and omits the section entirely when there are none", async () => {
    ...
    fetchMock.mockClear();
    await generateCarStoryProse({ ...baseInput, unconfirmedFindings: ["Pre-2020 service history"] }, "key");
  ...
  it("instructs the model never to judge the owner as a person, only the car", async () => {
    const fetchMock = vi.fn().mockResolvedValue(geminiResponse(JSON.stringify(validResult)));
    vi.stubGlobal("fetch", fetchMock);
    await generateCarStoryProse(baseInput, "key");
