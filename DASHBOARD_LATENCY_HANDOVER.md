...
**Still open**: the query above hasn't been re-run against live data yet since this fix - it needs a fresh deploy, then a few real tab clicks on the live site, then that `customEvents` query, before this is trustworthy. `tests/api/rum-tab-switch-route.test.ts` (6 cases) and the existing `DashboardShell.test.tsx` suite (46 cases) both still pass, and this was a genuine, portal-verified bug fix, not a guess - but "the route returns 200" was already true before this fix too, so re-verify the actual data lands this time rather than assuming the same fix-and-move-on pattern worked twice in a row.
