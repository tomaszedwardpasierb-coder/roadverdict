...
export interface BuyerOpinionInput {
  ...
  // Vehicle Check on this share link (see report/[token]/detailed's own
  // vdiUnlock handling) - an entirely different, official industry
  // check, not the DVLA/MOT facts above. Deliberately a small, named
  // subset of VdiCheckResult rather than the whole type: this opinion
  // only needs the handful of facts serious enough to change a dealer's
  // read, not the full report (which is shown in full, separately, by
  // VdiCheckSection further down the same page).
  vdiCheck?: {
    isStolen: boolean;
...
function buildFactsBlock(input: BuyerOpinionInput): string {
  ...
  if (input.vdiCheck) {
    ...
    lines.push(input.vdiCheck.hasOutstandingFinance ? "- Outstanding finance recorded against this bike" : "- No outstanding finance recorded");
