// Place at: src/lib/tracker/carReportQuestions.ts
//
// Car equivalent of reportQuestions.ts - identical logic, reusing the
// exact same metrics/thresholds from sellerReportVerdict.ts (already
// vehicle-neutral, not duplicated). Only one line actually differs from
// the bike version: "this car was listed" instead of "this bike was
// listed".
...
export function generateCarBuyerQuestions(m: SellerVerdictMetrics): string[] {
  ...
  if (m.recentRegistrationChangeDays !== null && m.recentRegistrationChangeDays <= RECENT_REGISTRATION_CHANGE_DAYS) {
    questions.push(`Ask why the registration changed ${m.recentRegistrationChangeDays} day${m.recentRegistrationChangeDays === 1 ? "" : "s"} before this car was listed.`);
  }
