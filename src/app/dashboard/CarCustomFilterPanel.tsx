// Place at: src/app/dashboard/CarCustomFilterPanel.tsx
//
// Car equivalent of CustomFilterPanel.tsx ("Look something up") - same
// standalone client-side lookup over already-loaded records, just
// against the car catalogs (CAR_JOB_LABELS/CAR_JOB_GROUPS,
// CAR_MOD_LABELS/CAR_MOD_GROUPS, CAR_BILL_LABELS) instead of the
// motorcycle ones. computeMPGSeries itself is shared/vehicle-agnostic
// (see mpgCalc.ts's own header comment), so it's imported directly
// rather than duplicated.
'use client';

import { useState } from 'react';
import { CAR_JOB_LABELS, CAR_JOB_GROUPS } from '@/lib/tracker/carJobTypes';
import { CAR_MOD_LABELS, CAR_MOD_GROUPS, CAR_MOD_LABEL_TO_KEY, findGroupForCarCategory } from '@/lib/tracker/carModTypes';
...
import { CarModSearchAutocomplete } from './CarModSearchAutocomplete';
...
export function CarCustomFilterPanel({ records, mods, bills, fuelLogs, currency, rates, fuelEconomyUnit }: Props) {
  ...
  function handleModSearch(label: string) {
    setModSearch(label);
    const matchedKey = CAR_MOD_LABEL_TO_KEY[label];
    if (matchedKey) {
      setModGroup(findGroupForCarCategory(matchedKey));
  ...
  if (category === 'service') {
    entries = records
      ...
      .map((r) => ({ date: r.date, description: CAR_JOB_LABELS[r.jobType] ?? r.jobType, cost: r.cost }));
  } else if (category === 'mods') {
    entries = mods
      .filter((m) => {
        if (modItem !== 'all') return m.category === modItem && inDateFilter(m.date);
        if (modGroup !== ALL_MODS_GROUP) {
          const groupData = CAR_MOD_GROUPS.find((g) => g.group === modGroup);
      ...
      .map((m) => ({ date: m.date, description: `${CAR_MOD_LABELS[m.category] ?? m.category}: ${m.name}`, cost: m.cost }));
  } else if (category === 'bills') {
    entries = bills
      .filter((b) => (billType === 'all' || b.billType === billType) && inDateFilter(b.date))
      .map((b) => ({ date: b.date, description: CAR_BILL_LABELS[b.billType] ?? b.billType, cost: b.cost }));
  } else {
    entries = fuelLogs.filter((f) => inDateFilter(f.date)).map((f) => ({ date: f.date, description: 'Fuel fill-up', cost: f.cost }));
  }

  // Computed on the FULL, unfiltered fuel log first - same reasoning as
  // CustomFilterPanel.tsx: filtering the raw logs by date before
  // computing segments would break the mileage-consecutive relationship
  ...
  const modGroupData = CAR_MOD_GROUPS.find((g) => g.group === modGroup);
  ...
  return (
    <div className={styles.chartCard} style={{ gridColumn: '1 / -1' }}>
      ...
      <div className="field">
        <label htmlFor="car-lookup-category">Category</label>
        <select id="car-lookup-category" value={category} onChange={(e) => handleCategoryChange(e.target.value as Category)}>
      ...
      {category === 'service' && (
        <div className="field" style={{ marginTop: '0.7rem' }}>
          <label htmlFor="car-lookup-service-item">Item</label>
          <select id="car-lookup-service-item" value={serviceJob} onChange={(e) => setServiceJob(e.target.value)}>
      ...
      {category === 'mods' && (
        <>
          <div className="field" style={{ marginTop: '0.7rem' }}>
            <label htmlFor="car-lookup-mod-search">Search for an item</label>
            <CarModSearchAutocomplete
              id="car-lookup-mod-search"
              value={modSearch}
              onChange={setModSearch}
              onSelect={handleModSearch}
              placeholder="e.g. dash cam, tow bar..."
            />
          </div>
          <div className="field" style={{ marginTop: '0.7rem' }}>
            <label htmlFor="car-lookup-mod-group">Group</label>
            <select id="car-lookup-mod-group" value={modGroup} onChange={(e) => handleModGroupChange(e.target.value)}>
              <option value={ALL_MODS_GROUP}>All modifications</option>
              {CAR_MOD_GROUPS.map((g) => (
          ...
          {modGroup !== ALL_MODS_GROUP && (
            <div className="field" style={{ marginTop: '0.7rem' }}>
              <label htmlFor="car-lookup-mod-item">Item</label>
              <select id="car-lookup-mod-item" value={modItem} onChange={(e) => setModItem(e.target.value)}>
                <option value="all">All in this group</option>
                {modGroupData?.subgroups.map((sg) => (
                  <optgroup key={sg.subcategory} label={sg.subcategory}>
                    {sg.mods.map((m) => (
                      <option key={m} value={m}>{CAR_MOD_LABELS[m]}</option>
                    ))}
                  </optgroup>
                ))}
              </select>
      ...
      {category === 'bills' && (
        <div className="field" style={{ marginTop: '0.7rem' }}>
          <label htmlFor="car-lookup-bill-item">Item</label>
          <select id="car-lookup-bill-item" value={billType} onChange={(e) => setBillType(e.target.value)}>
      ...
      <div className="field" style={{ marginTop: '0.9rem' }}>
        <label htmlFor="car-lookup-date-mode">Date range</label>
        <select id="car-lookup-date-mode" value={dateMode} onChange={(e) => setDateMode(e.target.value as 'range' | 'lastN')}>
      ...
      {dateMode === 'range' ? (
        <div style={{ display: 'flex', gap: '0.7rem', flexWrap: 'wrap' }}>
          <div className="field" style={{ marginTop: '0.5rem', flex: 1, minWidth: '140px' }}>
            <label htmlFor="car-lookup-from">From</label>
            <input id="car-lookup-from" type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} />
          </div>
          <div className="field" style={{ marginTop: '0.5rem', flex: 1, minWidth: '140px' }}>
            <label htmlFor="car-lookup-to">To</label>
            <input id="car-lookup-to" type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} />
          </div>
        </div>
      ) : (
        <div className="field" style={{ marginTop: '0.5rem', maxWidth: '160px' }}>
          <label htmlFor="car-lookup-last-n">Last how many days?</label>
          <input id="car-lookup-last-n" type="number" min="1" value={lastNDays} onChange={(e) => setLastNDays(e.target.value)} />
        </div>
      )}
