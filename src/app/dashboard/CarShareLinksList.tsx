// Place at: src/app/dashboard/CarShareLinksList.tsx
// Car mirror of ShareLinksList.tsx.
'use client';
...
export function CarShareLinksList({ links, carNames, appUrl }: Props) {
  ...
  async function handleDelete(token: string) {
    ...
    setBusyToken(token);
    try {
      const res = await fetch(`/api/cars/car-share-link/${token}`, { method: 'DELETE' });
  ...
  async function handleExtend(token: string) {
    setBusyToken(token);
    try {
      const res = await fetch(`/api/cars/car-share-link/${token}/extend`, {
        method: 'POST',
  ...
  function startEditingPrice(link: CarShareLinkDoc) {
  ...
  async function handleUpdatePrice(token: string) {
    ...
    setBusyToken(token);
    try {
      const res = await fetch(`/api/cars/car-share-link/${token}/asking-price`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div>
      {links.map((link) => {
        ...
        const url = `${appUrl}/car-report/${link.id}`;
        return (
          <div key={link.id} className={styles.jobCard} style={{ marginBottom: '0.8rem' }}>
            <div className={styles.jobCardTop}>
              <span className={styles.jobCardJob}>{carNames[link.carId] ?? 'Unknown car'}</span>
