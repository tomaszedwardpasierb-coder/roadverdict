...
export function ChartFilterBar() {
  const { range, setRange, viewBy, setViewBy, forecastMode } = useChartFilter();
  return (
    <>
      <ForecastToggle />
      {/* The real range/viewBy filters only apply to actual, already-logged
          data - in Forecast mode there's nothing for them to filter, so
          they'd otherwise sit there implying a control that does nothing. */}
      {!forecastMode && (
