// Place at: src/app/dashboard/CarTollCard.tsx
//
// Car equivalent of TollCard.tsx.
'use client';
...
export function CarTollCard({
  ...
  const { submit, submitting, error } = useTrackerFormSubmit(`/api/cars/car-tolls/${encodeURIComponent(toll.id)}`);
    ...
    return (
      <form className="ticket" onSubmit={handleSave} style={{ marginBottom: '0.7rem' }}>
        <div className="ticket__section">
          <div className="field">
            <label htmlFor={`edit-car-toll-date-${toll.id}`}>Date</label>
            <input id={`edit-car-toll-date-${toll.id}`} type="date" value={date} onChange={(e) => setDate(e.target.value)} required />
          </div>
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-toll-type-${toll.id}`}>Toll or charge</label>
            <select id={`edit-car-toll-type-${toll.id}`} value={tollType} onChange={(e) => setTollType(e.target.value)}>
          ...
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-toll-cost-${toll.id}`}>Cost ({symbol})</label>
            <input id={`edit-car-toll-cost-${toll.id}`} type="number" min="0" step="0.01" value={costDisplay} onChange={(e) => setCostDisplay(e.target.value)} required />
          </div>
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-toll-notes-${toll.id}`}>Notes</label>
            <textarea id={`edit-car-toll-notes-${toll.id}`} rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
          <AttachmentUploader value={attachment} onChange={setAttachment} idSuffix={`-car-toll-${toll.id}`} compareValues={{ cost: convertDisplayToGbp(Number(costDisplay), currency, rates), date }} />
        </div>
