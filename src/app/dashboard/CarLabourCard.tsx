// Place at: src/app/dashboard/CarLabourCard.tsx
//
// Car equivalent of LabourCard.tsx - the full-featured treatment (see
// LogCarLabourForm.tsx's own note on why), not the simplified pattern
// every other car card (e.g. CarModCard.tsx) uses.
'use client';
...
export function CarLabourCard({
  ...
  const { submit, submitting, error } = useTrackerFormSubmit(`/api/cars/car-labour/${encodeURIComponent(labour.id)}`);
  ...
  function handleCategorySearch(value: string) {
    ...
    const matchedKey = CAR_LABOUR_LABEL_TO_KEY[value];
    ...
    return (
      <form className="ticket" onSubmit={handleSave} style={{ marginBottom: '0.7rem' }}>
        <div className="ticket__section">
          <div className="field">
            <label htmlFor={`edit-car-labour-date-${labour.id}`}>Date</label>
            <input id={`edit-car-labour-date-${labour.id}`} type="date" value={date} onChange={(e) => setDate(e.target.value)} required />
          </div>
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-labour-search-${labour.id}`}>Search for a labour job</label>
            <input
              id={`edit-car-labour-search-${labour.id}`}
              type="text"
              list={`edit-car-labour-catalog-datalist-${labour.id}`}
              value={categorySearch}
              onChange={(e) => handleCategorySearch(e.target.value)}
              placeholder="e.g. brake bleed, timing belt, EV battery health check..."
            />
            <datalist id={`edit-car-labour-catalog-datalist-${labour.id}`}>
              {Object.keys(CAR_LABOUR_LABEL_TO_KEY).map((label) => (
                <option key={label} value={label} />
              ))}
            </datalist>
          </div>
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-labour-category-${labour.id}`}>Category</label>
            <select id={`edit-car-labour-category-${labour.id}`} value={category} onChange={(e) => setCategory(e.target.value)}>
          ...
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-labour-cost-${labour.id}`}>Cost ({symbol})</label>
            <input id={`edit-car-labour-cost-${labour.id}`} type="number" min="0" step="0.01" value={costDisplay} onChange={(e) => setCostDisplay(e.target.value)} required />
          </div>
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-labour-mileage-${labour.id}`}>Mileage ({unitLabel})</label>
            <input id={`edit-car-labour-mileage-${labour.id}`} type="number" min="0" value={mileageDisplay} onChange={(e) => setMileageDisplay(e.target.value)} required />
          ...
          <div className="field" style={{ marginTop: '0.9rem' }}>
            <label htmlFor={`edit-car-labour-notes-${labour.id}`}>Notes</label>
            <textarea id={`edit-car-labour-notes-${labour.id}`} rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
          <AttachmentUploader value={attachment} onChange={setAttachment} idSuffix={`-car-labour-${labour.id}`} compareValues={{ cost: convertDisplayToGbp(Number(costDisplay), currency, rates), date }} />
        </div>
  ...
  return (
    ...
    >
          ...
          {labour.mileageConflictWarning ? (
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  const res = await fetch(`/api/tracker/mileage-conflict-lookup?category=labour&id=${encodeURIComponent(labour.id)}&vehicleKind=car`);
      ...
      <div className={styles.jobCardTop}>
        <span className={styles.jobCardJob}>{CAR_LABOUR_LABELS[labour.category] ?? labour.category}</span>
      ...
      {showConflictModal && conflictReference && (
        <MileageConflictModal
          ...
          entryLabel={CAR_LABOUR_LABELS[labour.category] ?? labour.category}
