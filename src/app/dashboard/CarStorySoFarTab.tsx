// Place at: src/app/dashboard/CarStorySoFarTab.tsx
// Car mirror of StorySoFarTab.tsx.
'use client';
...
import type { CarUpcomingCostItem } from '@/lib/tracker/carUpcomingCosts';
...
export function CarStorySoFarTab({ carNickname, registration, currentMileage, distanceUnit, initialStory, sellerPrep }: Props) {
  ...
  async function handleGenerate() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/cars/car-story-so-far');
    ...
    return (
      <>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "0.75rem" }}>
          <h1 className={styles.heading}>The Story So Far{carTag}</h1>
          {mileagePill}
        </div>
        <div className={styles.storyIntro}>
        <p className={styles.subtext}>
          What your logged history says about this car - where it&apos;s strong, where a bit more logging would
        ...
        <div className={styles.storyReadinessNote}>
          ...
          attached, rather than straight after adding the car. If you run it too early, when there are only a
          handful of entries, the result is likely to have little or no real value. It may simply come back as
          &quot;Limited documentation&quot;, or produce a summary that doesn&apos;t tell the car&apos;s story
