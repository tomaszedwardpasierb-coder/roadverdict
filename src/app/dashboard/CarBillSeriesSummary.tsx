// Place at: src/app/dashboard/CarBillSeriesSummary.tsx
// Car mirror of BillSeriesSummary.tsx. BILL_SERIES_FREQUENCY_LABELS
// (billTypes.ts) and seriesTotalCost/seriesEndDate (billSeriesSchedule.ts)
...
import { BILL_SERIES_FREQUENCY_LABELS } from '@/lib/tracker/billTypes';
...
function SeriesRow({ series, currency, rates }: { series: CarBillSeriesDoc; currency: Currency; rates: ExchangeRates | null }) {
  const { submit, submitting, error } = useTrackerFormSubmit(`/api/cars/car-bill-series/${encodeURIComponent(series.id)}`);
    ...
    <div className={styles.card} style={{ marginBottom: '0.6rem' }}>
      <div className={styles.jobCardTop}>
        <span className={styles.jobCardJob}>
          {CAR_BILL_LABELS[series.billType] ?? series.billType} plan · {BILL_SERIES_FREQUENCY_LABELS[series.frequency]}
        </span>
...
export function CarBillSeriesSummary({
  series,
  currency,
  rates,
}: {
