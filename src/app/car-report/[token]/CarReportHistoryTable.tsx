// Place at: src/app/car-report/[token]/CarReportHistoryTable.tsx
// Car mirror of report/[token]/ReportHistoryTable.tsx. isBackdated/
// backdateNotice/isBeforeProduction are reused directly - genuinely
// vehicle-neutral (isBeforeProduction works via structural typing on
// {year, isCustomBuild}, which CarDoc also has).
'use client';
...
export function CarReportHistoryTable({
  rows,
  total,
  car,
  token,
  ...
  function isSelectable(r: CarReportRow): boolean {
  ...
  async function handleRemind(entryId: string) {
    ...
    try {
      const res = await fetch(`/api/car-report/${token}/remind`, {
        method: 'POST',
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/car-report/${token}/request-receipts`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          entryIds: [...selected],
  ...
  if (rows.length === 0) {
    return <p>No service, modification, or bill history has been logged for this car yet.</p>;
  }
      ...
      <table className={styles.table}>
        ...
        <tbody>
          {rows.map((r) => {
            ...
            const isPrePurchase = r.category === 'Modification' && isBeforeProduction(r.date, car);
            const isImage = r.attachment?.fileType === 'image/jpeg' || r.attachment?.fileType === 'image/png';
            const attachmentUrl = r.attachment ? `/api/cars/car-report-attachment/${token}/${encodeURIComponent(r.attachment.blobName)}` : null;
            const status = entryRequestStatus[r.id];
            ...
            return (
              <tr key={r.id}>
                <td>
                  {fmtDate(r.date)}
                  {isPrePurchase && (
                    <div className={styles.backdateNoteSoft}>Pre-purchase expense (bought before {car.year})</div>
                  )}
                  {backdated && (
