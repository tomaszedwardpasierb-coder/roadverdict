// Place at: src/app/api/cars/car-share-link/[token]/extend/route.ts
// Car mirror of api/tracker/share-link/[token]/extend/route.ts.
import { NextRequest, NextResponse } from "next/server";
import { getSession } from "@/lib/auth/session";
import { getCarShareLink, extendCarShareLink } from "@/lib/tracker/carShareLink";
...
export async function POST(request: NextRequest, props: { params: Promise<{ token: string }> }) {
  ...
  const updated = await extendCarShareLink(params.token, duration);
